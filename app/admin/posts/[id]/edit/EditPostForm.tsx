...
export default function EditPostForm({
  ...
  const dangerZone = (
    <button
      type="button"
      onClick={onDelete}
      className="inline-flex min-h-11 items-center text-sm text-red-600 hover:underline dark:text-red-400 sm:min-h-0"
    >
      이 글 삭제
    </button>
  )
    ...
    <form ref={formRef} action={handleAction} onChange={markDirty}>
      ...
      >
        <div className="space-y-6">
          ...
          <MarkdownEditor name="content" defaultValue={post.content} height={editorHeight} />
        </div>
      </PostEditorShell>
